...
//--------------------------------------------------------------
USB_CDC_STATUS_t USB_CDC_GetStatus(void)
{
...
//--------------------------------------------------------------
ErrorStatus USB_CDC_SendString(char *ptr, USB_CDC_LASTBYTE_t end_cmd)
{
  ...
  if(end_cmd==LFCR) {
    USB_VCP_DataTx(0x0A); // LineFeed senden
    USB_VCP_DataTx(0x0D); // CariageReturn senden
  }
  else if(end_cmd==NULLBYTE) {
      USB_VCP_DataTx(0x00); // Nullbyte senden
  }
...
//--------------------------------------------------------------
USB_CDC_RXSTATUS_t USB_CDC_ReceiveString(char *ptr)
{
  uint16_t check;
